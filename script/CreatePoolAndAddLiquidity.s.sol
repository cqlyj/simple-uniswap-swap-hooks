...
contract CreatePoolAndAddLiquidity is Script {
    ...
    IERC20 constant token0 =
        IERC20(address(0x779877A7B0D9E8603169DdbD7836e478b4624789)); // LINK
    IERC20 constant token1 =
        IERC20(address(0xFF34B3d4Aee8ddCd6F9AFFFB6Fe49bD371b8a357)); // DAI
    Currency constant currency0 = Currency.wrap(address(token0));
    Currency constant currency1 = Currency.wrap(address(token1));
    address hookAddress =
        Vm(address(vm)).getDeployment("OneTimeSwap", uint64(block.chainid));
    ...
    function run() external {
        ...
        PoolKey memory pool = PoolKey({
            currency0: currency0, // LINK
            currency1: currency1, // DAI
            fee: lpFee,
